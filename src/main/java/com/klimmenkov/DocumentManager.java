...
public class DocumentManager {
    ...
    public List<Document> search(SearchRequest request) {
        ...
        return storage.values().stream()
                .filter(doc -> titlePrefixes == null || titlePrefixes.isEmpty() ||
                        titlePrefixes.stream().anyMatch(prefix -> doc.getTitle().startsWith(prefix)))
                .filter(doc -> containsContents == null || containsContents.isEmpty() ||
                        containsContents.stream().anyMatch(content -> doc.getContent().contains(content)))
                .filter(doc -> authorIds == null || authorIds.isEmpty() || authorIds.contains(doc.getAuthor().getId()))
                .filter(doc -> createdFrom == null || !doc.getCreated().isBefore(createdFrom))
                .filter(doc -> createdTo == null || !doc.getCreated().isAfter(createdTo))
